...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/22 15:25:37 by tfarenga          #+#    #+#             */
/*   Updated: 2021/01/22 15:30:21 by tfarenga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
template <typename T>
static ft::ListIt<T> get_n_fwd(ft::List<T> &lst, size_t index)
{
	size_t i = 0;
	for (ft::ListIt<T> ite = lst.begin();
		 ite != lst.end(); ++ite)
		if (i++ == index)
			return ite;
	std::stringstream s;
	s << "Index " << index << " is out of bound!";
...
static void test_iterator()
{
	...
	assert(lst.size() == 1);
	{
		ft::ListIt<ConstrCounter> ite = lst.begin();
		++ite;
		assert(ite == lst.end());
	}
	...
	{
		ft::ListIt<ConstrCounter> ite = lst.end();
		--ite;
		--ite;
		--ite;
	...
	assert(ite->val == 1);
	{
		ft::ListIt<ConstrCounter> ite2 = lst.begin();
		++ite2;
		++ite2;
		assert(++ite == ite2);
	}
	...
	assert(ite == lst.end());
	{
		ft::ListIt<ConstrCounter> ite2 = lst.end();
		--ite2;
		assert(--ite == ite2);
	}
	assert(++ite == lst.end());
...
static void test_insert_one()
{
...

	{
		ft::ListIt<ConstrCounter> ite = lst.begin();
		++ite;
		ft::ListIt<ConstrCounter> ret =
			lst.insert(ite, ConstrCounter(5));
		ft::ListIt<ConstrCounter> exp = lst.begin();
		++exp;
		assert(ret == exp);
		assert(lst.size() == 2);
...
static void test_erase()
{
	...
	++ite;
	++ite;
	ft::ListIt<ConstrCounter> ret = lst.erase(ite);
	ft::ListIt<ConstrCounter> exp = lst.begin();
	++exp;
	++exp;
	assert(ret == exp);
...
static void test_erase_range()
{
	...
	ft::ListIt<ConstrCounter> ite1 = lst.begin();
	++ite1;
	ft::ListIt<ConstrCounter> ite2 = lst.end();
	--ite2;
	lst.erase(ite1, ite2);
